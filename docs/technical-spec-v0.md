...
`sima learn` runs worker → proposal → clean archivist → apply-ready gate → apply. `apply` still exists as a separate manual invocation, but learn uses the same apply-ready filter before mutating knowledge so decision and deterministic mutation gates stay distinct.
