...
`hybrid` means lexical fallback is used only when the embedding path is unavailable or errors (for example, the command is missing). If embedding runs successfully and returns zero matches above threshold, SIMA treats that as a valid empty result instead of falling through to permissive token overlap.
...
`top_k_per_part`/`min_part_score` provide bounded rescue below the global threshold. For decomposed tasks, SIMA can rescue the top match for each sub-query when at least one high-confidence match already cleared `min_score` somewhere in the same task. For single-topic tasks that produce no decomposition parts, SIMA can rescue the top whole-task match when it clears `min_part_score` and shares a non-generic lexical anchor with the task. Generic tokens such as `feature`/`фича` do not count as anchors, so branch-name collisions like `feature/x` do not reintroduce lexical fallback noise.
