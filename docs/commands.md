...
For decomposed tasks, `top_k_per_part` can rescue the top match for each sub-query when at least one high-confidence match already cleared `min_score` somewhere in the same task. `min_part_score` is a lower floor for those rescues. This anchored rescue helps short sub-queries such as `открыть PR` without reintroducing top-1 noise for completely unrelated single-intent tasks.
